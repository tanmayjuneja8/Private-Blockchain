    ...
    Temp.findOne({}, (err,foundarr)=>{
        ...
        if(selected!=null)
        {
            if(Array.isArray(selected))
            {
                ...
                BkChain.findOne({}, (err,foundchain)=>{
                    ...
                    selected.forEach((item)=>{
                        ...
                        const b = Blockchain.addBlock(foundchain.bchain[foundchain.bchain.length-1] , tobeinserted.key, tobeinserted.data);
            ...
            else
            {
                let x= temp[selected];
                BkChain.findOne({}, (err,foundchain)=>{
                    const b = Blockchain.addBlock(foundchain.bchain[foundchain.bchain.length-1] , x.key, x.data);
                    const newblock = new Bk(b);
                    newblock.save();
                    foundchain.bchain.push(newblock);
                    foundchain.save();
...
app.post('/ulogin', function(req,res){
    ...
    User.findOne({username : uid}, (err,founduser)=>{
        if(founduser)
        {
            ...
            {
                Temp.findOne({}, (err,foundarr)=>{
                    const newobj = new Obj({key:uid,data:info});
